...
@app.callback(
    # outputting graphs to the children
    dash.dependencies.Output('graphs', 'children'),
    [dash.dependencies.Input('vehicle-data-name', 'value')],
    # update the graphs at the inverval
    events=[dash.dependencies.Event('graph-update', 'interval')]
)
def update_graph(data_names):
    graphs = []
    update_obd_values(times, oil_temps, intake_temps,
                      coolant_temps, rpms, speeds, throttle_pos)

    # the class choice determines the size of the columns for the graph sizes
    if len(data_names) > 2:
        # column size s=small 12 columns, m=medium 6 columns
    ...
    for data_name in data_names:
        ...
        # return a list of these plotly graphs
